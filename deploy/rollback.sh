...
if [ -n "${added}" ]; then
    ...
    # failed one.
    #
    # --entrypoint alembic is NOT optional, and omitting it fails in the worst
    # possible way. The image's ENTRYPOINT is entrypoint.sh, which took no
    # arguments and hardcoded `alembic upgrade head`, so
    #     run --rm --no-deps app alembic downgrade <target>
    # silently discarded the command and RE-RAN THE UPGRADE THAT HAD JUST
    # FAILED. Observed on the box: the rollback re-raised the failing
    # migration's own error and froze at tier 3 with the site still serving the
    # broken container.
    "${COMPOSE[@]}" run --rm --no-deps --entrypoint alembic app downgrade "${target}" || freeze
fi
