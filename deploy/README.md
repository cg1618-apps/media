...
**Tier 2's downgrade must override the image entrypoint**, and the line reads
oddly without knowing why:

```bash
docker compose -f docker-compose.prod.yml run --rm --no-deps \
    --entrypoint alembic app downgrade <target>
```

`entrypoint.sh` runs `alembic upgrade head` and then uvicorn. Without
`--entrypoint`, `run app alembic downgrade <target>` hands those words to it as
arguments — and it re-runs **the upgrade that just failed** instead. The
failure is silent: the output looks like the migration failing a second time.
`entrypoint.sh` now runs a passed command rather than discarding one, so both
halves are covered; the explicit flag is not redundant, because it does not
depend on the image behaving.
...
`deploy.sh --ci` recovers from it: a detached HEAD at a commit `main`
**contains** returns to `main` before deploying, which is what the next deploy
was going to pull anyway. Any other non-`main` checkout still refuses. Without
that, one rollback would silently disable automatic deploys until somebody
noticed.
...
box's `HEAD` against `origin/main`. That also catches a deploy that failed
