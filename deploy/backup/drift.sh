...
# Diverged. The question is not "how new is main" but "how long has this box
# been missing something", so the clock starts at the OLDEST commit the box does
# not have rather than the newest.
#
# Measuring from the newest hides the exact failure this job exists to catch:
# every merge to main resets the age to zero, so a box that has stopped
# deploying entirely never alerts as long as somebody merges more often than the
# grace window. Active development would mask a dead runner indefinitely.
# Observed after a rollback left the box behind - the check reported "main is 0h
# old" and exited 0 while the box was genuinely stale.
#
# Still measured from the commit rather than from when this check first noticed,
# so a box that was off for a week reports the true age on its first run back
# instead of restarting the clock.
oldest_missing="$(git rev-list --reverse "${local_rev}..${remote_rev}" | head -1)"

# Empty means main is not ahead: the box holds commits main does not have. That
# is a checkout which has wandered off main rather than a deploy that has not
# run - a different fault with the same consequence, so it alerts immediately
# rather than waiting out a grace window that would never expire.
if [ -z "${oldest_missing}" ]; then
    echo "this box is at ${local_rev}" >&2
    echo "main is at      ${remote_rev}" >&2
    echo "The box is not behind main but does not match it - the checkout has" >&2
    echo "left main, or was rolled back and never brought back." >&2
    exit 1
fi

missed_at="$(git log -1 --format=%ct "${oldest_missing}")"
age_hours=$(( ( $(date +%s) - missed_at ) / 3600 ))

if [ "${age_hours}" -lt "${GRACE_HOURS}" ]; then
    echo "diverged, oldest undeployed commit is ${age_hours}h old - inside the ${GRACE_HOURS}h grace window"
    exit 0
fi

echo "this box is at ${local_rev}" >&2
echo "main is at      ${remote_rev}" >&2
echo "oldest undeployed commit ${oldest_missing} is ${age_hours}h old" >&2
