...
def log_deleted_record(db: Session, entry: Any, entry_type: str):
    ...
    try:
        ...
        if entry_type == "System Options":
            ...
            name_cn = _cn(entry, "game")
            if _has_cn(entry, "game"):
                name_en = _en(entry, "game")
            if getattr(entry, "series_id", None):
                s = db.query(Series).filter(Series.system_id == entry.series_id).first()
                series_cn = _cn(s, "series")
            if getattr(entry, "franchise_id", None):
        ...
        elif entry_type == "H-Comic":
            # Named through display_name, which already carries the type's
            # CN -> EN -> Alt -> JP/KR chain; the _cn helper knows no KR name.
            name_cn = entry.display_name or None
            name_en = getattr(entry, "h_comic_name_en", None)
            if getattr(entry, "series_id", None):
                s = db.query(Series).filter(Series.system_id == entry.series_id).first()
                series_cn = _cn(s, "series")
            if getattr(entry, "franchise_id", None):
