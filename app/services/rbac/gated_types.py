...
# by its migration and by the lifespan seed (gated_labels.ensure_label),
# attached to every entry of the type on every write path, and granted to no
# mode but `unrestricted` - see ensure_access_mode_seed, which keeps it off the
# other all-labels mode.
