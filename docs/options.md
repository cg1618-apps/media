...
any session outside `unrestricted` that means no `h_comic_*` or `h_game_*`
keys at all, no `H-Comic` or `H-Game` in `franchise_type`, no `h-comic` or
`h-game` in `media_type`, no `club` in `person_role`, and none of the three H
Genre categories in `option_categories`.
...
| `FranchiseType` (Enum) | `Anime`, `Movie`, `TV`, `Cartoon`, `Comic`, `ACG`, `Novel`, `Game`, `H-Comic`, `H-Game` | backend-internal only | not served |
| `FRANCHISE_TYPES` | `ACG`, `Anime Movie`, `TV`, `Movie`, `Cartoon`, `Comic`, `Novel`, `Game`, `H-Comic`, `H-Game` | `franchise.franchise_type` dropdown. `H-Comic` and `H-Game` are the types code branches on: a franchise carrying one carries that gated type's label and only entries of its family resolve into it ([entry-types.md](entry-types.md#franchise_type-values)) | `franchise_type` |
...
| `H_COMIC_USEFULNESS` | `非常實用`, `實用`, `特定情況實用`, `不實用` | `user_media_list.usefulness` (personal, on h-comic and h-game) and the `status` field of the `h_comic_highlights` and `h_game_highlights` note sections | `h_comic_usefulness` |
...
built from lists: `franchise_type` carries `Game`, `H-Comic` and `H-Game`,
and `media_type` carries `game`, `h-comic` and `h-game`. The four h-comic
vocabularies are served under `h_comic_`-prefixed keys, and the five h-game
ones under `h_game_`-prefixed keys, for the same one-flat-map reason.
Every h-game vocabulary is checked on every write path - a value outside it
is a 422 through the API, and dropped (logged) by the Sheets parser.
...
type plus `series`, `franchise`, `collection`; "Entries" means the eleven
...
plain bookmarks and games inherit it; `guide_resources` (`structured`, in
the 資源&工具 card,
...
**The 攻略 sections are fifteen sections across five cards rather than one
section with a `kind`**, because
...
`CREDIT_ROLES` filtered to `target == "person"`, which excludes both company
keys. `club` is the circle an h-comic comes out of: a person row, so artists
can belong to it through `person_membership`; its only scope is the gated
h-comic type.
...
manga / novel / comic and 作畫 / Illustrator / Artist / 繪師 for `illustrator`
on manga / novel / comic / h-comic. One vocabulary, several words.
...
| `original_source` | Original Source | `Platform` | tv-show, cartoon, movie, h-comic |
...
The three H genre fields are shared by the gated types, and serve gated types
alone, so their **category itself is a connection**: every value in `H Genre
Plot`, `H Genre Appearance` or `H Genre Relation` is hidden from a session
that can see none of those types, whatever scope rows it has - an unscoped, unused value
...
h-game. They have no legacy sheet header; h-comic and h-game credits and tags
all surface under their own keys.
...
`cartoon`, `manga`, `novel`, `comic`, `game`, `h-comic`, `h-game`. `OWNER_TYPE_KEYS` adds the grouping
...
| `H Genre Plot` | h-comic, h-game | tag field `h_genre_plot`; admin-managed, ships empty |
| `H Genre Appearance` | h-comic, h-game | tag field `h_genre_appearance`; admin-managed, ships empty |
| `H Genre Relation` | h-comic, h-game | tag field `h_genre_relation`; admin-managed, ships empty |
