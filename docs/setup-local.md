...
### Per-machine notes (`CLAUDE.local.md`)

Optional, and gitignored like `.env`. A `CLAUDE.local.md` in the project root
is read by Claude Code alongside `CLAUDE.md`, and it is where a machine says
which machine it is — this project is developed on two of them, **home** and
**company**, and the difference matters often enough that guessing is worse
than a three-line file. Write one per machine; never copy another machine's,
and never commit it. What the two machines actually differ in is
