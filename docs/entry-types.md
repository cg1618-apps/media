...
| External id / link | `mal_id` / `mal_link` (Tenrai) | `mal_id` / `mal_link` (Tenrai) | `imdb_id` / `imdb_link` (TMDB + OMDb) | `imdb_id` / `imdb_link` (TMDB + OMDb) | `imdb_id` / `imdb_link` (TMDB + OMDb) | `mal_id` / `mal_link` (Tenrai) | `mal_id` / `mal_link` (Tenrai) | `comicvine_id` / `comicvine_link` (Comic Vine) | `igdb_id` / `igdb_link` (IGDB), plus `steam_appid` / `steam_link` (Steam) — the appid is normally adopted from IGDB's `external_games` and is what Steam and the derived SteamDB link both key off | `mal_id` / `mal_link` (Tenrai), plus `ehentai_link` (E-Hentai: the cover and the illustrator credit, keyed by the gallery id and token in the link); Tenrai runs first, so the cover is MAL's, else E-Hentai's | `igdb_id` / `igdb_link` (IGDB) and `steam_appid` / `steam_link` (Steam), as game, plus `dlsite_link_jp` / `dlsite_link_tw` (DLsite: release date, studio and cover, keyed by the product id in the link); the cover comes DLsite, then Steam, then IGDB | `mal_id` / `mal_link` (Tenrai: airing status, release date and cover only) |
...
| | `anime` | `anime-movie` | `movie` | `tv-show` | `cartoon` | `manga` | `novel` | `comic` | `game` | `h-comic` | `h-game` | `hentai` |
|---|---|---|---|---|---|---|---|---|---|---| --- |---|
| Id extractor | `apply_extract_mal_id_anime` | `apply_extract_mal_id_anime` | `apply_extract_imdb_id` | `apply_extract_imdb_id` | `apply_extract_imdb_id` | `apply_extract_mal_id_manga_novel` | `apply_extract_novel_ids` (`apply_extract_mal_id_manga_novel` then `apply_extract_openlibrary_id`) | `apply_extract_comicvine_id` | `apply_extract_game_ids` (IGDB then Steam) | `apply_extract_mal_id_manga_novel`, as manga | `apply_extract_game_ids`, as game | `apply_extract_mal_id_anime` |
| Fill eligible when | `mal_id` set and missing values | `mal_id` set and missing values | missing values | missing values | `airing_type` in `{"Movie", "TV"}` and missing values | `mal_id` set and missing values | `mal_link` set and missing MAL values, **or** `mal_link` unset and `openlibrary_id` set and missing Open Library values | `comicvine_id` set and missing values | `igdb_id` set and missing IGDB values, **or** `steam_appid` set and Steam has written nothing yet | `mal_id` set and `serialization_status`, `release_date`, `end_date` or the cover missing, or `ch_total` on a `完結` KR entry, **or** `ehentai_link` names a gallery and the cover or the illustrator credit is missing | as game | `mal_id` set and `airing_status`, `release_date` or the cover missing |
| Fill function | `autofill_anime_from_mal` | `autofill_anime_movie_from_mal` | `autofill_movie_from_imdb` | `autofill_tv_show_from_imdb` | `autofill_cartoon_from_imdb` | `autofill_manga_from_mal` | `autofill_novel_from_mal` / `autofill_novel_from_openlibrary` (routed on `mal_link`) | `autofill_comic_from_comicvine` | `autofill_game_from_igdb` then `autofill_game_from_steam` | `_fill_h_comic`: `autofill_h_comic_from_mal` (Tenrai's manga record, no AniList or ratings) then `autofill_h_comic_from_ehentai` (the cover and the illustrator), all fill-only | game's two, generalised over the table: IGDB writes only `studio`, `game_genre`, `game_theme` and h_game's columns; Steam only prices and achievements | `autofill_hentai_from_mal` |
| Pause between calls | `MAL_PAUSE` (1 s) | 1 s | none | none | none | 1 s | 1 s | `COMICVINE_PAUSE` (1 s) + hourly budget | `STEAM_PAUSE` (0.5 s) + Steam storefront budget | `MAL_PAUSE` (1 s) | as game | `MAL_PAUSE` (1 s) |
...
| Bulk Replace selects | rows with `mal_id` or `mal_link` | rows with `mal_id` or `mal_link` | rows with `imdb_id` or `imdb_link` | rows with `imdb_id` or `imdb_link` | TV/Movie rows with `imdb_id` or `imdb_link` | rows with `mal_id` or `mal_link` | rows with `mal_id` or `mal_link` | **no bulk Replace** (`replace=None`) | rows with `steam_appid`, `steam_link`, `igdb_id` or `igdb_link` (IGDB fill-only, then Steam) | rows with `mal_id`, `mal_link` or `ehentai_link` | as game | rows with `mal_id` or `mal_link` |
| In Fill All / Replace All | yes / yes | yes / yes | yes / yes | yes / yes | yes / yes | yes / yes | yes / yes | **no / no** (`in_fill_all=False`, `in_replace_all=False`) | yes / yes | yes / yes | yes / yes | yes / yes |
...
`PIPELINES["h-comic"]` reads Tenrai's manga record - manga's endpoint and mapper - for the columns `h_comic` has: `serialization_status`, `release_date` and `end_date` (fill-only), the cover (downloaded only when the entry has none), and `ch_total` on a KR entry once its serialization is `完結`. MAL reports no page count, so `page_total` is never filled, and there are no rating columns to write. The gallery in `ehentai_link` is a second source, run after Tenrai: `autofill_h_comic_from_ehentai` fills the cover and, from the gallery's `artist:` tags, the illustrator (繪師) credit, only where they are still empty; an entry with only an E-Hentai link is filled too. It writes no date and no name - see [external-apis.md](external-apis.md#e-hentai). Every run and the single-entry write hook end in `run_sync_h_comic`, which clears each region's unused columns, and `run_sync_gated_labels`, which keeps the `h-comic` label on every entry and every `H-Comic` franchise.

`PIPELINES["hentai"]` reads Tenrai's anime record - the same endpoint and mapper anime uses, which serves Rx titles - for **three fields and two links only**: `airing_status` and `release_date` (fill-only), the cover (downloaded only when the entry has none), and the `Official site` and `Twitter` reference `media_source` rows (added only where the entry has none). Names, studio, scores and AniList are not written. Every run and the single-entry write hook end in `run_sync_hentai` (system options) and `run_sync_gated_labels`, which keeps the `hentai` label on every entry and every `Hentai` franchise.
