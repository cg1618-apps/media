...
| | `anime` | `anime-movie` | `movie` | `tv-show` | `cartoon` | `manga` | `novel` | `comic` | `game` | `h-comic` | `h-game` | `hentai` |
|---|---|---|---|---|---|---|---|---|---|---| --- |---|
| Id extractor | `apply_extract_mal_id_anime` | `apply_extract_mal_id_anime` | `apply_extract_imdb_id` | `apply_extract_imdb_id` | `apply_extract_imdb_id` | `apply_extract_mal_id_manga_novel` | `apply_extract_novel_ids` (`apply_extract_mal_id_manga_novel` then `apply_extract_openlibrary_id`) | `apply_extract_comicvine_id` | `apply_extract_game_ids` (IGDB then Steam) | `apply_extract_mal_id_manga_novel`, as manga | `apply_extract_game_ids`, as game | `apply_extract_hentai_ids` (MAL then AniDB) |
| Fill eligible when | `mal_id` set and missing values | `mal_id` set and missing values | missing values | missing values | `airing_type` in `{"Movie", "TV"}` and missing values | `mal_id` set and missing values | `mal_link` set and missing MAL values, **or** `mal_link` unset and `openlibrary_id` set and missing Open Library values | `comicvine_id` set and missing values | `igdb_id` set and missing IGDB values, **or** `steam_appid` set and Steam has written nothing yet | `mal_id` set and `serialization_status`, `release_date`, `end_date` or the cover missing, or `ch_total` on a `完結` KR entry | as game | `mal_id` set and `airing_status`, `release_date` or the cover missing, **or** AniDB enabled, `anidb_id` set and one of the same three missing |
| Fill function | `autofill_anime_from_mal` | `autofill_anime_movie_from_mal` | `autofill_movie_from_imdb` | `autofill_tv_show_from_imdb` | `autofill_cartoon_from_imdb` | `autofill_manga_from_mal` | `autofill_novel_from_mal` / `autofill_novel_from_openlibrary` (routed on `mal_link`) | `autofill_comic_from_comicvine` | `autofill_game_from_igdb` then `autofill_game_from_steam` | `autofill_h_comic_from_mal`: Tenrai's manga record, fill-only, no AniList or ratings | game's two, generalised over the table: IGDB writes only `studio`, `game_genre`, `game_theme` and h_game's columns; Steam only prices and achievements | `autofill_hentai_from_mal`, then `autofill_hentai_from_anidb` for what MAL left blank |
| Pause between calls | `MAL_PAUSE` (1 s) | 1 s | none | none | none | 1 s | 1 s | `COMICVINE_PAUSE` (1 s) + hourly budget | `STEAM_PAUSE` (0.5 s) + Steam storefront budget | `MAL_PAUSE` (1 s) | as game | `MAL_PAUSE` (1 s), AniDB's own 4 s spacing, and a stop on an AniDB error answer |
...
| Bulk Replace selects | rows with `mal_id` or `mal_link` | rows with `mal_id` or `mal_link` | rows with `imdb_id` or `imdb_link` | rows with `imdb_id` or `imdb_link` | TV/Movie rows with `imdb_id` or `imdb_link` | rows with `mal_id` or `mal_link` | rows with `mal_id` or `mal_link` | **no bulk Replace** (`replace=None`) | rows with `steam_appid`, `steam_link`, `igdb_id` or `igdb_link` (IGDB fill-only, then Steam) | rows with `mal_id` or `mal_link` | as game | rows with `mal_id`, `mal_link`, `anidb_id` or `anidb_link` |
...
`PIPELINES["h-comic"]` reads Tenrai's manga record - manga's endpoint and mapper - for the columns `h_comic` has: `serialization_status`, `release_date` and `end_date` (fill-only), the cover (downloaded only when the entry has none), and `ch_total` on a KR entry once its serialization is `完結`. MAL reports no page count, so `page_total` is never filled, and there are no rating columns to write. Every run and the single-entry write hook end in `run_sync_h_comic`, which clears each region's unused columns, and `run_sync_gated_labels`, which keeps the `h-comic` label on every entry and every `H-Comic` franchise.

`PIPELINES["hentai"]` reads Tenrai's anime record - the same endpoint and mapper anime uses, which serves Rx titles - for **three fields and two links only**: `airing_status` and `release_date` (fill-only), the cover (downloaded only when the entry has none), and the `Official site` and `Twitter` reference `media_source` rows (added only where the entry has none). Names, studio, scores and AniList are not written. **AniDB** then fills whichever of the three and the `Official site` row MAL left blank, keyed on `anidb_id` from `anidb_link` - the cover above all, since MAL misses many hentai OVAs; it is fill-only, so MAL's value wins, and it runs only while `ANIDB_CLIENT` / `ANIDB_CLIENTVER` are set (see [external-apis.md](external-apis.md#anidb)). Every run and the single-entry write hook end in `run_sync_hentai` (system options) and `run_sync_gated_labels`, which keeps the `hentai` label on every entry and every `Hentai` franchise.
