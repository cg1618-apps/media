...
travels as JSON text. `Person Membership` follows `Person Role`: both of its
...
4. For every non-blank data row:
   ...
   - **After the tab commits**, two passes, each over the whole table and each
     ...
     - When the tab is one of `GATED_LABEL_INVARIANT_TABS` - every gated
       type's entry tab (`H-Comic`), `Franchise`, `Content Label`,
       ...
       every `H-Comic` franchise). A hand-edited sheet that dropped a label
