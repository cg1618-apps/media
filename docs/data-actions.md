...
once a day, and treats any error answer but a not-found as a stop: the spec's
`budget` turns false and the run ends with the rest reported as left for the
next run. See [external-apis.md](external-apis.md#anidb).
