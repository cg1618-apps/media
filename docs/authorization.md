...
|---|---|---|---|---|---|
| `media_type.anime`, `anime-movie`, `movie`, `tv-show`, `cartoon`, `manga`, `novel`, `comic`, `game`, `h-comic`, `hentai` (all eleven) | yes (as seeded) | yes (as seeded) | yes | implicit | — |
...
- **"As seeded" is what a fresh database gets.** The seed derives every
  ...
  `h-comic`, `hentai` - reaches `super` (locked on) and `admin` (implicit) and
  not `guest` or `user` until an admin grants it. The type axis is not what
  hides a gated type from a narrow session anyway: its label is (see
...
- **`borderline` is seeded identically to `unrestricted` and then diverges.**
  ...
  [Gated types](#gated-types)): `h-comic` and `hentai` belong to `unrestricted` alone,
...
- **Content labels are admin-created, with one system label.** On a fresh
  database the label row is empty for every mode except for `h-comic` and
  `hentai`, which the lifespan seeds and grants to `unrestricted` only — and a label minted
...
- **A fresh database carries two labels, `h-comic` and `hentai`.** Every other content
...
- `/api/auth/me` returns `is_admin`, `username`, `role`, `is_root`,
  ...
  session may see (`gated_types.visible_gated_types`) - `["h-comic", "hentai"]`
  in `unrestricted`, the one whose label a mode carries, `[]` otherwise. It names only the seeable
...
| `apply_media_visibility(query, db, viewer)` | anything spanning every type at once | The same two gates over the `media` supertable rather than one detail table: the media-type check becomes an `IN` over the types the viewer holds, and the label anti-join goes through `media_content_label.media_id` and `media.franchise_id`. The profile page needs this — it answers for all eleven types in one query. The query must already select from or join `Media` |
...
- **A scope naming a gated type.** A media type named in
  ...
  credits are hidden. The gated types are `h-comic` and `hentai`, so a person
...
- **A category serving gated types only.** A vocabulary value's own
  ...
  Appearance` and `H Genre Relation` - shared by h-comic and hentai - is hidden
  from a session that can see neither, even with no scope row and no use. A
  session that sees one of the two keeps the categories. A category shared with an
...
them; today it is `{"h-comic": "h-comic", "hentai": "hentai"}`. A session can see a gated type
...
| Surface | Left out |
|---|---|
| `GET /api/constants` | a vocabulary serving only hidden gated types (`TYPE_ONLY_VOCABULARIES`: `h_comic_region` and `h_comic_animation_status` with h-comic, `hentai_source_material` with hentai, `h_comic_originality` and `h_comic_usefulness` only when both are hidden), its key from `media_type`, a franchise type stamped only for it from `franchise_type` (`H-Comic`, `Hentai`), a person role scoped only to hidden types from `person_role` (`club`), a category serving only hidden types from `option_categories` / `tag_categories` (the H Genre categories, when both are hidden) |
...
routes, including the four per-type write routes across all eleven media types.
...
franchise of its franchise type (`h-comic` / `H-Comic`, `hentai` / `Hentai`)
...
the eleven `*Response` schemas declare their status field `Optional[str] = None`
...
| `GET /api/content-labels/`, `POST`, `PATCH`, `DELETE` | 409 duplicate key; delete cascades assignments (entries become visible again); 204. `DELETE` of a label a gated type requires (`h-comic`, `hentai`) is **409** |
| `GET/PUT /api/content-labels/entry/{media_type}/{entry_id}` | list / replace an entry's label keys; 400 unknown type, 404 entry, 422 unknown label, 422 a set without the required label on a gated entry (`h-comic` on an h-comic, `hentai` on a hentai) |
| `GET/PUT /api/content-labels/franchise/{franchise_id}` | the same for a franchise; 422 a set without `h-comic` on a franchise whose type includes `H-Comic`, or without `hentai` on one whose type includes `Hentai` |
