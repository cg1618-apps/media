...
The development machines pin the same value for the same reason.
...
| | |
| --- | --- |
| `~/media/.env` | Everything the containers read. Mode 600, gitignored. |
...
**That window is measured from the oldest commit the box is missing**, not from
the newest commit on `main`, and the distinction is what makes the check work.
Ageing the newest restarts the clock on every merge, so a box that had stopped
deploying entirely would never alert while anybody was merging more often than
the window — active development masking a dead runner. A box that is not behind
`main` but still differs has no oldest-missing commit and no window that could
expire, so it alerts at once: the checkout has left `main`.
...
and the frontend build on every pull request, and has no deployment job.
Deploying is a second workflow, `deploy.yml`.

It does **not** run again on the push to `main`, because the repository ruleset
requires a branch to be up to date before it merges. The check therefore always
ran against the base as it is, so the tree that lands is the tree that was
tested and a second run would test it twice.
...
`~/actions-runner/_work/...`, and the stack only works from `~/media` —
...
| Postgres (`media`) | **23 MB** | whole database including indexes, ~11,000 rows |
