...
| `navigation.js` | `NAV_SECTIONS` (Library mega-panel, Track, Insights, then Entry, Note and Admin with `requires: "admin"`), `activeItem`, `visibleSections(sections, has, canSeeType)` (filters rows as well as sections — Insights carries two `requires: "admin"` rows, and the Library's H-Comic and H-Game rows carry `gatedType: "h-comic"` / `"h-game"`, each dropped unless `canSeeType` says yes for its own type; left out, every gated row is dropped). |
...
| Nav rows (Library → ACG → H-Comic, H-Game) | `gatedType` on each item in `config/navigation.js`; `visibleSections(sections, has, canSeeType)` drops it. Both permission surfaces ask the same helper, and `navigation.test.js` pins the pair |
