...
- **`components/info`** — `InfoCard` (+`InfoRow`), `NamingCard`, `ScoreBlock`,
  ...
  column-backed
  `malLink`/`anidbLink`/`imdbLink`/`comicvineLink`/`openLibraryLink`/`igdbLink` props
...
- **`components/forms`** — `FormField`, `ComboBox` (`onSelect(id, label)`),
  ...
  *selected*, a pick past it replacing the oldest; single-value tag fields
  ...
  picker that sits among other cells. Used by every entry Add/Modify tab, the
  person, character, publisher and studio forms, `QuoteForm`, `MemeForm` and,
