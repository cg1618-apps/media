...
| h-comic, h-game, hentai | `Ep` (never shown) | **no** — `WHOLE_ONLY_TYPES` (a hentai entry is one episode); each offered in the editor's type filter only to a session that can see it |
