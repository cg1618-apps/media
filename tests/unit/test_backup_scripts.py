...
def test_install_reports_the_timers_it_actually_enabled():
    # The summary used to name its two timers in prose. When media-drift.timer
    # was added the loop enabled it and the summary kept saying two - so the
    # operator was told to run two jobs by hand while three had been enabled,
    # and the third sat unarmed with its Healthchecks check grey. Observed on
    # the box.
    #
    # Same maintained-list hazard as the shellcheck glob and the SCHEDULE map:
    # the loop is complete and the prose beside it is not, and nothing makes
    # them disagree loudly. So the summary must be BUILT from what the loop did.
    body = (BACKUP_DIR / "install.sh").read_text(encoding="utf-8")
    assert "ENABLED+=(" in body, "install.sh must collect what it enables"

    summary = body[body.index("Enabled now:") :]
    named = re.findall(r"media-[a-z]+\.timer", summary.split("\n")[0])
    assert not named, (
        f"the 'Enabled now' line names timers in prose: {named}. "
        "Build it from the ENABLED array instead."
    )
