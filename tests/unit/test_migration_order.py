...
ARCHIVE = Path(__file__).resolve().parents[2] / "alembic" / "versions_archive"
...
def test_the_archived_revisions_are_out_of_the_chain():
    ...
    # Assert the property, not a count standing in for it.
    #
    # This was `len(revisions) < 10`, true when the chain held nine and meant as
    # a proxy for "nothing has been moved back from the archive". A proxy like
    # that fails on the tenth legitimate migration, for the wrong reason, with a
    # message blaming the archive when nothing was moved - and the only way past
    # it is to edit the number, which teaches the next author that the assertion
    # is noise. Found by adding a tenth revision.
    ...
    for path in ARCHIVE.glob("*.py"):
        ...
        if found:
            archived.add(found.group(1))

    # Without this the check is vacuous: a wrong ARCHIVE path or a regex that
    # matches nothing yields an empty set, and an empty set intersects nothing.
    assert len(archived) > 100, (
        f"only {len(archived)} archived revision ids parsed from {ARCHIVE} - "
        "the path or the pattern is wrong, and this would pass against anything"
    )

    in_chain = archived & {rev.revision for rev in revisions}
    assert not in_chain, (
        f"archived revisions are back in the chain: {sorted(in_chain)}. They "
        "live in alembic/versions_archive/ because they cannot build a database "
        "from empty; Alembic must not be able to resolve them."
    )
