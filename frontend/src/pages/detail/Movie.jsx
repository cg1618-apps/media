...
export default function Movie() {
  ...
  const movieQuery = useMediaItem("movie", publicId);
  useCanonicalPath("movie", movieQuery.data);
  ...
  const { setMediaItem, fetchMediaItem, invalidateMedia } =
    useMediaCacheUpdate("movie", publicId);
  ...
  const franchise = useMemo(
    () =>
      movie?.franchise_id
        ? franchises.find((f) => f.system_id === movie.franchise_id) || null
        : null,
    [franchises, movie?.franchise_id],
  );
  const series = useMemo(
    () =>
      movie?.series_id
        ? seriesList.find((s) => s.system_id === movie.series_id) || null
        : null,
    [movie?.series_id, seriesList],
  );
  const loading =
    movieQuery.isLoading || franchiseQuery.isLoading || seriesQuery.isLoading;
  const error =
  ...
  const imdbScore =
    movie.imdb_rating && movie.imdb_rating !== "N/A" ? movie.imdb_rating : "—";
      ...
      {isAdmin && (
        <div className="border border-border-strong border-dashed px-3 py-2 flex flex-wrap gap-3 items-center justify-between mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={() => navigate(`/modify?id=${system_id}&type=movie`)}>
              Quick edit
            </Button>
            <Button
              onClick={async () => {
        ...
        <div className="lg:col-span-3 space-y-10">
          ...
          <header>
            ...
            {/* What restricts this entry, if anything. Its franchise's
                ...
                changed. */}
            <ContentLabelChips labels={movie.content_labels} className="mb-6" />
