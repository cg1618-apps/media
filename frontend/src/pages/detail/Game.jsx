...
import { publisherLabel, publisherValue, studioValue } from "../../components/info/StudioLinks";
...
export function GameProgress({ game }) {
  ...
  const pct = hasAchievements ? Math.min(100, Math.round((earned / Number(total)) * 100)) : null;
  ...
  return (
    <Slip title="Copies">
      <div className="space-y-1.5">
        {sorted.map((copy, i) => (
          <div key={copy.system_id || i} className="flex items-center gap-2 flex-wrap">
            {copy.storefront && <Chip className="shrink-0">{copy.storefront}</Chip>}
            {copy.ownership && <Chip className="shrink-0">{copy.ownership}</Chip>}
            {copy.copy_format && <span className="text-sm text-text">{copy.copy_format}</span>}
            {copy.acquisition && (
              <span className="text-xs text-text-faint">{copy.acquisition}</span>
            )}
            {copyPrice(copy) && (
              <span className="text-xs font-mono text-text-faint">{copyPrice(copy)}</span>
            )}
            {copy.acquired_date && (
              <span className="text-xs font-mono text-text-faint">{copy.acquired_date}</span>
            )}
            {copy.remark && <span className="text-xs text-text-faint">{copy.remark}</span>}
          </div>
        ))}
      </div>
    </Slip>
...
export default function Game() {
  ...
  const { setMediaItem, fetchMediaItem, invalidateMedia } = useMediaCacheUpdate("game", publicId);
  ...
  const franchise = useMemo(
    () =>
      game?.franchise_id ? franchises.find((f) => f.system_id === game.franchise_id) || null : null,
    [franchises, game?.franchise_id]
  );
  const series = useMemo(
    () => (game?.series_id ? seriesList.find((s) => s.system_id === game.series_id) || null : null),
    [game?.series_id, seriesList]
  );

  const loading = gameQuery.isLoading || franchiseQuery.isLoading || seriesQuery.isLoading;
  const error =
    gameQuery.error?.message || franchiseQuery.error?.message || seriesQuery.error?.message || null;
  ...
  if (error || !game) {
    return <MediaLoadingState error={error || "Game not found"} errorTitle="Error Loading Game" />;
  }

  const titleMain = getDisplayName(game, "game");
  const titleSub = game.game_name_en && game.game_name_en !== titleMain ? game.game_name_en : null;
  ...
  const eyebrow = ["Game", game.game_type, game.release_status, game.completion_level, year]
    .filter(Boolean)
    .join("  ·  ");

  return (
    // One provider around the whole page, because the notes are rendered in
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                onClick={async () => {
                  if (!isAdmin) return;
                  try {
                    const res = await fetch(endpoints.resource("game").complete(system_id), {
                      method: "POST",
                      credentials: "include",
                    });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3 space-y-10">
            ...
            {/* Progress and 待辦 Todo in one card. How far in I am and what I
              still mean to do are the same question asked twice, and they
              were two cards a page apart - so the backlog was read after
              every other note rather than beside the playtime it belongs to.
              GameProgress renders nothing when there is no playtime and no
              achievement total, which leaves the card holding the todo lists
              alone; that is fine, and better than an empty Progress card. */}
              ...
              <InfoCard
                ...
                fields={[
                  ...
                  [
                    ...
                    {
                      label: "Copies",
                      value: game.copies?.length ? String(game.copies.length) : null,
                    },
                  ],
                ]}
              />
