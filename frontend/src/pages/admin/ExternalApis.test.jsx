...
describe("ExternalApis", () => {
  ...
  it("says which entries a Replace run would rewrite", async () => {
    ...
    //
    // findByText inside, not getByText. ExternalApis has no loading guard, so
    // the <ul> renders immediately and empty while `data` is undefined and
    // `overwritten` is []. Awaiting the CONTAINER therefore resolves at once
    // against an empty list, and a synchronous query inside it then fails
    // whenever the mocked fetch has not resolved yet - green on a fast machine,
    // red on a loaded CI runner.
    const summary = await screen.findByTestId("overwrite-summary");
    expect(await within(summary).findByText(/mal_rating/)).toBeInTheDocument();
  });
